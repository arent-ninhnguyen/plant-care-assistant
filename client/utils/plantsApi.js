...
// Use environment variable for the base API URL, fallback to 5001
const BASE_API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001/api';
const PLANTS_API_URL = `${BASE_API_URL}/plants`; // Construct the specific endpoint URL
...
const plantsApi = {
  ...
  getPlants: async () => {
    try {
      ...
      console.log('Making GET request to:', PLANTS_API_URL);
      // Use axios directly with dynamic headers and absolute URL
      const response = await axios.get(PLANTS_API_URL, { headers });
      return response;
    } catch (error) {
  ...
  getPlant: async (id) => {
    try {
      const headers = await getAuthHeaders();
      const response = await axios.get(`${PLANTS_API_URL}/${id}`, { headers });
      return response;
    } catch (error) {
  ...
  addPlant: async (plantData) => {
    try {
      ...
      const response = await axios.post(PLANTS_API_URL, plantData, { headers });
      return response;
    } catch (error) {
  ...
  updatePlant: async (id, plantData) => {
    try {
      ...
      if (plantData instanceof FormData) {
        headers['Content-Type'] = 'multipart/form-data';
        response = await axios.put(`${PLANTS_API_URL}/${id}`, plantData, { headers });
      } else {
  ...
  deletePlant: async (id) => {
    try {
      const headers = await getAuthHeaders();
      const response = await axios.delete(`${PLANTS_API_URL}/${id}`, { headers });
      return response;
    } catch (error) {
